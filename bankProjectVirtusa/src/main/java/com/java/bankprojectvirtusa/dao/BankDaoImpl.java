package com.java.bankprojectvirtusa.dao;



import com.java.bankprojectvirtusa.model.Accounts;
import com.java.bankprojectvirtusa.util.ConnectionHelper;
...
public class BankDaoImpl implements BankDao {
  ...
  public int generateAccountNo() throws SQLException, ClassNotFoundException {
    ...
    String cmd = "select case when max(AccontNo) IS NULL THEN 1 " +
      " else max(AccontNo)+1 end accno from account";
  ...
  public String createAccount(Accounts account) throws SQLException, ClassNotFoundException {
    ...
    conn = ConnectionHelper.getConnection();
    String cmd = "Insert into account(AccontNo, FirstName, LastName, City, State, Amount, CheqFacil, AccountType) " +
      " values(?,?,?,?,?,?,?,?)";
    pstmt = conn.prepareStatement(cmd);
  ...
  @Override
  public Accounts searchAccount(int AccontNo) throws SQLException, ClassNotFoundException {
    conn = ConnectionHelper.getConnection();
    String cmd = "select * from account where AccontNo = ?";
    pstmt = conn.prepareStatement(cmd);
    pstmt.setInt(1, AccontNo);
    ResultSet rs = pstmt.executeQuery();
    Accounts accounts = null;
    if (rs.next()) {
      accounts = new Accounts();
      accounts.setAccountNo(rs.getInt("AccontNo"));
  ...
  public String updateAccount(int accountNo, String city, String state) throws SQLException, ClassNotFoundException {
    ...
    if (accountFound !=null){
      conn = ConnectionHelper.getConnection();
      String cmd = "Update account set City = ?, State = ? WHERE AccontNo = ?";
      pstmt = conn.prepareStatement(cmd);
  ...
  public String closeAccount(int accountNo) throws SQLException, ClassNotFoundException {
    ...
    if (account !=null){
      conn = ConnectionHelper.getConnection();
      String cmd = "Update account set status = 'inactive' where AccontNo = ?";
      pstmt = conn.prepareStatement(cmd);
  ...
  public String depositAccount(int accountNo, double amount) throws SQLException, ClassNotFoundException {
    ...
    if (accounts !=null){
      conn = ConnectionHelper.getConnection();
      String cmd = "Update account set Amount = Amount + ? where AccontNo = ?";
      pstmt = conn.prepareStatement(cmd);
      pstmt.setDouble(1, amount);
      pstmt.setInt(2, accountNo);
      pstmt.executeUpdate();
      cmd = "Insert into Trans(AccontNo, TransAmount, TransType) values(?,?,?)";
      pstmt = conn.prepareStatement(cmd);
      pstmt.setInt(1, accountNo);
